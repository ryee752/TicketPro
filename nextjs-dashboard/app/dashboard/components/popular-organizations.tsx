...
export default function EventList() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      const params = new URLSearchParams({ //passes parameters to api/home/organization/popular-organizations/route.tsx
        user_id: login.id //Unlike upcoming-events.tsx, this should be the ID of a regular user currently signed in (not organization) which is passed to route.ts
      });
      const response = await fetch(`/api/home/user/popular-organizations?${params.toString()}`);
    
    // const response = await fetch(`/api/home/organization/popular-organizations`);
